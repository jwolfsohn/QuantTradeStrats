...
def run_daily_macro_check():
    ...
    logging.info("Executing End-of-Day Macro, VRP, and TSMOM Evaluation...")

    # Guard: if morning close never ran (bot was down at 09:35), close overnight SPY now
    # before recalculating to avoid double-stacking the overnight weight into SPY
    stale_check = load_master_state()
    if stale_check.get("_OVERNIGHT_SPY_ACTIVE", 0.0) > 0.0:
        logging.warning("Overnight SPY flag still active at EOD — morning close was missed. Closing now.")
        run_morning_overnight_close()
    ...
    try:
        ...
        # Map OVERNIGHT_SPY into physical SPY position for Alpaca
        if "OVERNIGHT_SPY" in macro_targets:
            overnight_weight = macro_targets.pop("OVERNIGHT_SPY")
            macro_targets["SPY"] = macro_targets.get("SPY", 0.0) + overnight_weight
            state["_OVERNIGHT_SPY_ACTIVE"] = overnight_weight
...
def run_hourly_statarb_check():
    ...
    if not is_market_open() and "--now" not in sys.argv:
        logging.info("Market is closed. Skipping intraday StatArb execution.")
        return

    logging.info("Starting Hourly StatArb hunt...")
    logging.info("Executing Intraday StatArb Evaluation...")
    try:
...
def run_morning_overnight_close():
    """Run at 09:35 ET to close out the Night Effect SPY exposure."""
    if not is_market_open() and "--now" not in sys.argv:
        logging.info("Market is closed. Skipping morning overnight close.")
        return

    logging.info("Starting Morning Check: Removing Overnight SPY exposure...")
    state = load_master_state()
    
    overnight_weight = state.get("_OVERNIGHT_SPY_ACTIVE", 0.0)
    if overnight_weight > 0.0:
        current_spy = state.get("SPY", 0.0)
        state["SPY"] = max(0.0, current_spy - overnight_weight)
        state["_OVERNIGHT_SPY_ACTIVE"] = 0.0
        
        save_master_state(state)
        print_state(state)
        rebalance_to_target(state)
    else:
        logging.info("No overnight SPY active flag found. Skipping reduction.")

def print_state(state):
    logging.info("-" * 30)
    logging.info("CURRENT MASTER SECURE STATE WEIGHTS:")
...
for day in [schedule.every().monday, schedule.every().tuesday, 
    ...
    day.at("09:35", "America/New_York").do(run_morning_overnight_close)
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--now":
        ...
        logging.info("Bot decoupled scheduler started.")
        logging.info("- StatArb bounds checking: Every hour at X:15")
        logging.info("- Macro/VRP/TSMOM evaluation: Weekdays at 15:45 ET")
        while True:
